module Levels (Level(lStart,lLength,lMap),Cell(Full,Empty),getLevels) where
...
levelgroup _ chr = case chr of
                     "" -> False
                     _  -> True

getLevels = do
        lines <- lines <$> readFile "levels.txt"
        let levels' = groupBy levelgroup lines
